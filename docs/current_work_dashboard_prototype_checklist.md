...
- [ ] Add QA, provenance, and deployment decisions after the first working prototype exists.
Outcome: source links, notes, change-log visibility, and hosting choices are handled with a working interface in hand instead of abstract speculation.
Sets up: the project can decide whether to stay local-first a bit longer or move to a hosted static site once the prototype proves out.
Concrete QA findings queued for this item (found 2026-07-12 while reviewing the live league filter):
...
- [ ] LAUNCH BLOCKERS (2026-07-13) — the complete list of what still stands between this project and a public, "complete" v1, excluding style/content polish. Hosting decision is made and documented in `docs/launch_guide.md` (free static website; no downloadable app).
  1. **Sync the Google Sheets master with the local workbook** (~30 min, manual). The local xlsx is ahead of Sheets: apply the replication CSVs in `data/processed/workbook_import/post_import_fixes/` (club-ID remaps, caps fills, birthplace enrichment, GK columns), fix Avazbek Ulmasaliev's `place_of_birth` → `Fergana`, and move the five legacy `club_corr_*` rows from `sources` into `change_log`. Until this is done the Sheets master is not the source of truth it claims to be.
  2. **First git commit + push to GitHub** (~5 min). The repo has zero commits — nothing is version-controlled yet, which is both a launch blocker (git-connected hosting) and a data-loss risk.
  3. **Connect the host and smoke-test the live URL** (~15 min). Netlify or Cloudflare Pages with base `site`, build `pnpm build`, publish `site/dist`; then click through all seven routes on the live URL (deep links like `/stats` exercise the already-in-place `_redirects` SPA fallback).
  4. **Post-tournament data refresh** (time-bound, ~1–2 h after the July 19 final). Re-run the two Wikipedia scrapers and the three FIFA captures, rebuild import sheets, update the workbook, refresh exports, push. Launching before the semis with "through quarterfinals" data is fine; the dataset is only *complete* once the final's results are in.
  5. **Two players still lack birthplaces** (Mohammad Taha, Cho Wi-je — Wikidata has nothing; needs a manual source). Cosmetic-level data gap, listed for completeness.
Outcome: a public URL non-technical people can open, backed by a Sheets master that matches what's deployed, with the full tournament recorded.
Sets up: v1.0 — after this, everything else (images, fuzzy search, confederation sort, code-splitting, new insights) is enhancement, not completion.
